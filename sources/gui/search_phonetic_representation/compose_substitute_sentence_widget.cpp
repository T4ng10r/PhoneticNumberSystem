...
{
  bool bResult = false;
  bResult      = QObject::connect(save_subtitution, SIGNAL(clicked(bool)), public_part, SLOT(on_save_clicked(bool)));
...
void ComposeSubstituteSentenceWidgetPrivate::set_subtitute()
{
  ...
  for (WordResultWidget* result_widget : word_results_container) {
    MatchingWord matchingWord = result_widget->get_current_word();
    substituteWords.push_back(matchingWord.getWord().c_str());
  }

  gDataThread->save_substitute(substituteWords, searchNumber);
}
...
void ComposeSubstituteSentenceWidget::on_word_selected(int end_index)
{
  ...
  _pimpl->logger.log(log4cplus::DEBUG_LOG_LEVEL,
                     str(boost::format("Next word should start from '%1%' digit of searched number ( %2%)") %
                         end_index % _pimpl->searchNumber.substr(end_index - 1)));
...
void ComposeSubstituteSentenceWidget::setSearchNumber(std::string searchNumber) { _pimpl->searchNumber = searchNumber; }
